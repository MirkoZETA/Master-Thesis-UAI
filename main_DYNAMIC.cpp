...
int main(int argc, char* argv[]) {
    ...
    // Abrir archivo
    file.open("./results/EuroCore_CLE_ORIGINAL_FirstFit.csv", std::ios_base::app);
    ...
    for (int lambda = 2500; lambda <= 8000; lambda+=500) {
        ...
        std::cout << "Lambda: " << lambda 
                  << " - Blocking Probability: " << sim.getBlockingProbability() 
                  << " - BBP: " << BBP(totalBitRate, bloqueadosBitRate, pesoBitRate)
                  << std::endl;
    }
    file.close();
    return 0;
